...
import { ref } from 'vue'
import { AddressType } from '@/components/city/city'
defineProps<{
  goods: GoodsInfo
}>()
// 假设获取了用户的个人信息，有常用地址列表
const userAddress = ref('山东省 德州市 禹城市')
// 选择的地址
const changeAddress = (address: AddressType) => {
  userAddress.value = `${address.provinceName} ${address.cityName} ${address.countyName}`
}
</script>
<template>
